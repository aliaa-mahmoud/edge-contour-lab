...
from core.snake.greedy_algorithm import evolve_snake, initialize_circular_contour


class _SnakeCanvasInteractor(QObject):
	"""Attach circle-drawing and overlay rendering to a QLabel from main.ui."""

	circle_drawn = pyqtSignal(tuple, int)
	...
	def __init__(self, label: QLabel, pixmap_factory, parent=None):
		...
		# Install event handling after state is initialized to avoid early resize events
		# touching attributes before they exist.
	...
	def eventFilter(self, obj, event):
		...
		if event.type() == QEvent.MouseMove and self._drawing:
			p = self._widget_to_image(event.pos())
			if p is not None:
				self._edge = p
				self._refresh(preview=True)
		...
		if event.type() == QEvent.MouseButtonRelease and event.button() == Qt.LeftButton and self._drawing:
			self._drawing = False
			if self._center is not None and self._edge is not None:
				cx, cy = self._center
				ex, ey = self._edge
				radius = int(round(((ex - cx) ** 2 + (ey - cy) ** 2) ** 0.5))
				if radius >= 3:
					self.circle_drawn.emit((cx, cy), radius)
	...
	def _refresh(self, preview: bool = False):
		...
		if preview and self._center is not None and self._edge is not None:
			c = self._image_to_display(*self._center)
			e = self._image_to_display(*self._edge)
			if c and e:
				dx = e[0] - c[0]
				dy = e[1] - c[1]
				rr = int(round((dx * dx + dy * dy) ** 0.5))
				painter.setPen(QPen(Qt.green, 2, Qt.DashLine))
				painter.drawEllipse(QPoint(c[0], c[1]), rr, rr)
...
class SnakeController(MainController):
	...
	def bind_ui(self, ui):
		...
		self._canvas.circle_drawn.connect(self._on_circle_drawn)
	...
	def load_image(self, path: str):
		...
		self._ui.snakeStatusLbl.setText("Draw initial circle on image")
		self.contours_changed.emit(None, None)

	def _on_circle_drawn(self, center: tuple[int, int], radius: int):
		if self._image is None or self._ui is None:
			return
		self._initial_contour = initialize_circular_contour(
			center,
			radius,
			num_points=self._ui.snakePointsSpin.value(),
		)
		self._final_contour = None
		...
		self._ui.snakeStatusLbl.setText(f"Circle ready: center={center}, radius={radius}")
	...
	def clear_initial_contour(self):
		...
		if self._ui is not None:
			self._ui.snakeRunBtn.setEnabled(False)
			self._ui.snakeClearBtn.setEnabled(False)
			self._ui.snakeStatusLbl.setText("Circle cleared")
	...
	def run(self, params: dict):
		if self._image is None or self._initial_contour is None:
			self.error_occurred.emit("Load image and draw an initial circle first.")
			return
